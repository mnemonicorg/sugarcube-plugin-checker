...
const plugin = (envelope, {log, cfg}) => {
  ...
  const checkUnits = async units => {
    ...
    const found = await collect(videosList(key), chunk(50, uids))
      .then(flatten)
      // if the youtube api thows error, ignore for now and try again at next check
      .catch(e => {
  ...
  const d = filter(u => sourceType.includes(u._sc_source), data);

  return checkUnits(d) // check the units in the envelope for youtube videos
    .then(r => {
      log.info(
